...
- Tests: `pip install -r requirements-dev.txt`, then `pytest` from `backend/` (security regression suite in `tests/`, runs against a throw-away SQLite DB in a temp dir; CI runs it too). No linter or formatter is configured.
...
The backend has a security-focused pytest suite (`backend/tests/`); the frontend has no test runner — verify frontend changes by running the app (see the `run` skill) and exercising the affected flow manually.
...
- Security building blocks (each has a docstring explaining the threat it addresses): `request_limits.py` (ASGI body-size middleware, mirrored by the Next proxy in `frontend/src/app/api/[...path]/route.ts` — keep both in sync), `upload_validation.py` + `safe_archive.py` (upload/ZIP-import limits, name sanitizing), `pattern_safety.py` + `pattern_runner.py` (learned regex patterns are untrusted: static checks, then run in a killable subprocess), `totp.py` (2FA + recovery codes, login flow in `routers/users.py`), `audit.py` (`AuditLog` table; call `audit.log_event` for any new security-relevant action, never put secrets in `detail`), `secrets_crypto.py` (settings encrypted with a key derived from `SECRET_KEY`), `user_data.py` (complete account erasure incl. files), `http_utils.py` (Content-Disposition/ICS escaping). Secrets (SMTP/backup passwords) are never returned by GET endpoints — only `*_set` flags.
